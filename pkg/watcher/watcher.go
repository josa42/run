...
func NewMatcher(pattern []string) Matcher {
	...
	for _, p := range pattern {
		m.globs = append(m.globs, glob.MustCompile(p))
	}
	return m
}
...
func (dw *Watcher) stopped() <-chan struct{} {
	return dw.ctx.Done()
}
...
func (dw *Watcher) dirTree(queryRoot string) <-chan string {
	...
	go func() {
		...
		err := filepath.Walk(queryRoot, func(path string, f os.FileInfo, err error) error {
			if !f.IsDir() || filepath.Clean(path) == filepath.Clean(queryRoot) {
				return nil
			}
			found <- path
			return nil
